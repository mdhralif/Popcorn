...
export default function DetailPage() {
    ...
    function getNewVisionMovie(movieId) {
        movieService.getMovieById(movieId).then(result => {
            console.log("Movie data received:", result.data);
            setMovie(result.data);
        });
        actorService.getActorsByMovieId(movieId).then(result => {
            console.log("Actors data received:", result.data);
            setActors(result.data);
        });
        cityService.getCitiesByMovieId(movieId).then(result => {
            console.log("Cities data received:", result.data);
            setCinemaSaloons(result.data);
        });
    ...
    function addState(movieTime) {
        ...
        let movieDto = {
            ...
            imageUrl: movie.movieImageUrl || movie.imageUrl,
    ...
    <div style={{paddingTop: '80px'}}>
            ...
            <div className=' container mt-5'>
                <div className='row gx-0 pt-2 justify-content-center align-items-start'>
                    <div className='col-sm-12 col-md-6 text-center mb-4' >
                        <img className='img-thumbnail w-50' src={movie?.movieImageUrl || movie?.imageUrl} alt={movie?.movieName} />
                    </div>
                    <div className='col-sm-12 col-md-6 text-start text-light'>
                        <h3>{movie?.movieName}</h3>
                        <hr/>
                        <h5>Director: {movie?.directorName}</h5>
                        <h5>Actors: {actors?.map((actor, index) => (
                            <span key={actor.actorId || index}>
                                {actor.actorName}
        ...
        <style dangerouslySetInnerHTML={{
            __html: [
                ...
                'z-index: -1;',
                'inset: 0;',
                `background-image: url(${movie?.movieImageUrl || movie?.imageUrl});`, 
            ...
            <div className='container'>
                <div className='row justify-content-between ms-0 ms-md-5 ps-0 ps-md-5'>
                    <div className='col-sm-4 text-start'>
                        <p> <strong> Release Date: </strong> {movie.releaseDate ? dateConvert(movie.releaseDate) : 'N/A'}</p>
                        <p> <strong>Duration: </strong>{movie.duration || 'N/A'} Minutes</p>
                        <p><strong>Genre: </strong>{movie.categoryName || 'N/A'}</p>
                    </div>
                    <div className='col-sm-8 text-start'>
                        <p><strong>Plot: </strong>{movie.description || 'No description available'}</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section className='p-5'>
            ...
            <h3 className='text-center mb-4'>Other Movies Currently Showing</h3>
            <Swiper
                slidesPerView={5}
                spaceBetween={0}
                pagination={{
            ...
            >
                {otherMovies.map(movie => (
                    <SwiperSlide key={movie.movieId} >
                            ...
                            <img src={movie.movieImageUrl || movie.imageUrl}
                                class="img-fluid mx-2" alt={movie.movieName} />
                        </div>
                    </SwiperSlide>
                ))}
            </Swiper>
            ...
            <div class="modal-dialog modal-sm modal-dialog-centered modal-dialog-scrollable" >
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {cinemaSaloons && cinemaSaloons.length > 0 ? (
                            cinemaSaloons.map((saloon, index) => (
                                <a key={saloon.cityId || index} className='text-start text-dark' href='#!'
                            ...
                            <p className="text-muted">No cities available for this movie</p>
                        )}
                    </div>
